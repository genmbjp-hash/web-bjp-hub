...
interface PosyanduPageProps {
  entities: Entity[];
  onSelectEntity: (entity: Entity) => void;
  onBack: () => void;
}

export const PosyanduPage: React.FC<PosyanduPageProps> = ({ entities, onSelectEntity, onBack }) => {
  const posyanduEntities = entities.filter((e) => e.category === 'Kesehatan');
  ...
  return (
    <div className="pb-10">
      <Container className="pt-6 space-y-6">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-stone-900">Posyandu BJP</h1>
          <p className="text-xs sm:text-sm text-stone-500 mt-1">
            Layanan kesehatan, Posyandu, Posbindu, dan tanggap darurat warga Komplek Bintara Jaya Permai (RW 11).
          </p>
        </div>

        {posyanduEntities.length === 0 ? (
          <div className="text-center py-16 space-y-3 bg-white rounded-2xl border border-stone-200">
            ...
            <h3 className="font-bold text-stone-700">Belum Ada Layanan Kesehatan</h3>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
            {posyanduEntities.map((entity) => (
